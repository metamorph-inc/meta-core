...
def invoke(focusObject, rootObject, componentParameters, **kwargs):
    ...
    shutil.copy(os.path.join(meta_path, 'bin', 'run_CTH.sh'), output_dir)
    ...
    componentParameters['runCommand'] = 'bash run_CTH.sh'
    if focusObject.CopyResults:
        componentParameters['runCommand'] += ' --keep-files'
    componentParameters['labels'] = 'CTH'
    # testing on windows:
