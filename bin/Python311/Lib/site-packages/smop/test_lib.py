...
c = deg2rad(rad2deg([1 2]))""", 'trig')
        expected = _make_arrays({'a': [[.1, .5]], 'b': [[.2, 1]], 'd': .2, 'c': [[1, 2]]})
        # this breaks in 1.15; see smop.test_core:Getitem.test07
        # self.assertEquals(res, expected)
        for index in sorted(expected):
            numpy.testing.assert_almost_equal(res[index].tolist()[0], [expected[index]] if isinstance(expected[index], float) else expected[index].tolist()[0])
