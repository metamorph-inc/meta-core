...
def _conv_to_threejs(filename, filename_wire, surf):
    """
    Convert the provided surface representation to three.js format.

    Convenience method provided for backwards compatibility. Allows JSON queries to return geom
    without having to save a temp file.

    ``color`` can be set 3 ways:
    - By passing in a user-specified RGB triplet as list
     - None (the object is white by default)
     - 'auto' -
    """
    output = {"version": 2,
              "materials": [
    ...
    faces = []
    tris = np.array(surf["tris"], dtype=np.int32)
    for ti, t in enumerate(tris):
        faces += [0] + t.tolist()
    verts = []
    for i in xrange(len(surf["x"])):
        verts.extend([float(surf["x"][i]),
                      float(surf["y"][i]),
                      float(surf["z"][i])])
    output["faces"] = faces
...
def save_threejs(filename, filename_wire, surf):
    """
    Save the provided geometry as three.js compatible file
    """
    output = _conv_to_threejs(filename, filename_wire, surf)
    with open(filename, 'w') as f:
        json.dump(output, f, indent=4)


if __name__ == "__main__":
    srf = open_geom(r"D:\NPZ\a023_led_aux_lamp_black_prt.npz")
    save_threejs(r"D:\test.js", r"D:\test_wire.js", srf)
