...
def save_ascii_stl(filename, solidname, surf):
    """
    Save 'surf' as ascii stl file to 'filename'
    """
    x, y, z, tris = [surf[k] for k in ["x", "y", "z", "tris"]]
    
    with open(filename, "wb") as stl_out:
        ## Write out an 80 byte header
        stl_out.write('solid {0}\n'.format(solidname))
        
        ## Each triangle has a normal (not important -> written as zeros) and 3 nodes v0, v1, v2
        for v0, v1, v2 in tris:
            stl_out.write('facet normal 0.0 0.0 0.0\n')
            stl_out.write('    outer loop\n')
            
            stl_out.write('        vertex {0} {1} {2}\n'.format(x[v0], y[v0], z[v0]))
            stl_out.write('        vertex {0} {1} {2}\n'.format(x[v1], y[v1], z[v1]))
            stl_out.write('        vertex {0} {1} {2}\n'.format(x[v2], y[v2], z[v2]))
            stl_out.write('    endloop\n')
            stl_out.write('endfacet\n')
            
        stl_out.write('endsolid {0}\n'.format(solidname))
