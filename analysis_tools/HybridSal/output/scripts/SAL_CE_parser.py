...
- `parse_assignments() -- for assignment blocks inside counterexample steps or global constraints
...
def parse_assignments(lines, lineNo=0, input_vars=None):
    ...
    for line in it:
        ...
        if assignM:
            ...
            if varsM:
                if varsM.group(3):
                    assigns[varsM.group(1)] = varsM.group(3).strip()
...
[{
  "GroupTitle": <title>,
  "GroupBody": [
    "Global Constraints: <accumulated assignments>",
    "Step <N>:",
...
~~~ When adding a scope then limit to these variables from LTL:
    ...
    for i in iterator:
        ...
        # parse global constrains if any:
        if line.startswith('Global Constraints'):
            constraints = True
            assignment_lines = ["--- Global Constraints (assignments) ---"]
            continue
        if constraints:
            if line.startswith('Path'):
                constraints = False  # turn constrainst OFF again
                # print collected constraints assignments
                assignments, _ = parse_assignments(assignment_lines)
                group_body.append("{0}{1}".format("{0:03} -- ".format(lineNo) if lineNo else "", assignments[0]))
                if lineNo: lineNo += 1
                assignment_lines = []
            elif line and not line.startswith('='):  # skip empty or delimiting lines
                assignment_lines.append(line) # collect assignment lines
            continue

        # skip blank or certain lines (unless parsing global constraints):
        if not line or line.startswith(('=','Path')):
            logger.debug("  -- skipping: {}".format(line))
            continue

        # now try to parse the steps from Path:
