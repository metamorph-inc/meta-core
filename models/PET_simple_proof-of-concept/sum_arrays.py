...
if __name__ == '__main__':
    ...
    def unwrap(val):
        if isinstance(val, six.text_type) or isinstance(val, str):
            return float(val)
    ...
    for metric in testbench_manifest['Metrics']:
        if isinstance(params[metric['Name']], numpy.ndarray):
            metric['Value'] = numpy.sum(params[metric['Name']])
        else:
            metric['Value'] = params[metric['Name']]
        # metric['Value'] = numpy.ones(10) * params[metric['Name']] + 2
