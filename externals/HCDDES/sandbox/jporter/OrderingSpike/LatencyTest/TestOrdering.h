...
public:
	...
	unsigned long getValue( unsigned long idx ) const {
		return l[idx].val();
        }

        void getValues( std::vector< unsigned long long > & vals ) const {
		for ( unsigned long idx = 0; idx < l.size(); idx++ )
		{
			vals.push_back( l[idx].val() );
                }
	}
};
