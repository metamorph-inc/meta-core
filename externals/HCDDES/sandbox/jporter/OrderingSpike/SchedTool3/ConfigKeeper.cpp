...
ConfigKeeper::ConfigKeeper(void) : _desc("Allowed options") {
	_desc.add_options()
		...
		("verbose,v", po::value< bool >()->default_value(false), "Show detailed information.")
		("debug,d", po::value< bool >()->default_value(false), "Show debug information.")
		("select,s",po::value< vector<string> >(&_dbgList), "Select entities (procs/buses) to schedule.") 
