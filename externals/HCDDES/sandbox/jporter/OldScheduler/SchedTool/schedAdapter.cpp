...
{
	std::string errMsg;
	std::string task = std::string(procname) + "/" + std::string(taskname);

	double f = freq * FreqU2Val( frequnits );
	double t = dur * TimeU2Val( durunits );
	if ( t > 1/f )
		errMsg = "  !! Task " + task + ": Duration is too large for frequency.";
	cout << task << ": ";
	cout << frqspec << f << " ";
	cout << t << errMsg << endl;

	_pModel->AddTask( string( procname ), Task( string(taskname), string(procname), f, t, (string(frqspec) == "=") ? true : false ));
}
				...
				char * sndtask )
{
	...
	_pModel->AddMsg( string( busname ), Msg( string(msgname), string(busname), s, string(sndtask) ));
}
...
void SchedAdapter::AddLatency( double latency, char * timeunits, 
