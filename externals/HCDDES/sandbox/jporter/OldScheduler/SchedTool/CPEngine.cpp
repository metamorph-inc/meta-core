...
// CP Solver Engine
CPEngine::CPEngine( InstanceKeeper * pIK, unsigned long long min, unsigned long long max ) : 
	...
	_pIK( pIK )
{
	...
	while (loop)
	{
		...
		IntArgs h(vdv.size()), m(vdv.size()), l(vdv.size());
...
CPEngine::CPEngine( InstanceKeeper * pIK, unsigned long long min, unsigned long long max, bool dbg ) : 
	vars( this, pIK->NumVars(), (int) min, (int) max ),  // instantiate constraint vars
	latencies( this, pIK->NumLatencies()+1, 0, (int) 2 * max ),
	...
	_pIK( pIK )
{
	...
	while (loop)
	{
		...
		linear( this, a, v, IRT_EQ, lrel.offset, lin_cstr[lrel.group], ICL_BND );
		dbg_idx++;
	...
	while (loop)
	{
		...
		linear( this, a, v, IRT_LQ, lrel.offset, lin_cstr[lrel.group], ICL_BND );
...
void CPEngine::CollectResults( InstanceKeeper * pIK, bool debug )
{
	...
	else
	{
		...
		{
			cout << "Linear group " << idx << ": " << pIK->getLinearGroup(idx) << " " << ((lin_cstr[idx].val() == 0) ? string("false") : string("true")) << endl;
			if ( lin_cstr[idx].val() == 1 )
				pIK->SaveConstraintResult( pIK->getLinearGroup(idx) );
		}
