	...
	try
	{
	  if(interactive)
	  {
		...
		try
		{
			...
			Udm::StaticDataNetworkSpecifier sdns_ESMoL_717( "DummyName", &dngBackend);
			std::vector< Udm::StaticDataNetworkSpecifier> dnsvec;
			dnsvec.push_back( sdns_ESMoL_717);
			// Create the project
			Udm::StaticUdmProject prj( dnsvec, ESMoL_2_SchedConfig::diagram);
			dnsvec.push_back( sdns_ESMoL_717);
...
#else
			// Calling the main entry point
			Udm::DataNetwork& eSMoL_ref_718= prj.GetDataNetwork( "DummyName");
			CUdmApp::UdmMain(&eSMoL_ref_718,currentObject,selectedObjects,param);
